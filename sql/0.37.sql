begin;
-- Добавляем к менеджеру направление синхронизации
alter table partner_manager add column "type" boolean not null default false;

-- Добавляем сообщению источник (id партнера или NULL, если сообщение свое)
alter table "Message" add column "partner_id" integer;
alter table "Message" add constraint partner_fkey foreign key("partner_id")
references partner(id) match simple on update cascade on delete set null;

-- Добавляем информацию о партнере в представление сообщения
...
SELECT m.id AS message_id, m.partner_id as partner, m.title AS message_title, m.message AS message_text, m.date_add AS message_date, m.subdomain_id, s.url AS subdomain,
m.user_id  user_id,
...
update config set value='0.37' where key='db_version';
commit;
