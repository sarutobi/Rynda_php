-- FIX 243 Введение справочника "тип сообщения"
begin;
create table message_type(
id serial,
name character varying(100),
primary key (id));
alter table message_type owner to wwwrynda;
insert into message_type(id, name) values(0,'Информация');
insert into message_type(name) values('Просьба о помощи');
insert into message_type(name) values('Прeдложение помощи');
insert into message_type(name) values('Актуальная рекомендация');
insert into message_type(name) values('Комментарий');

alter table "Message" add column "message_type" integer;
update "Message" set message_type=(flags & 240::bigint) >> 4;
alter table "Message" add foreign key(message_type) references message_type(id) on delete restrict;

-- Изменение представления сообщениий
...
update config set value='0.29' where key='db_version';
commit;
