-- Изменение представления сообщений - добавление поля идентификатор местоположения 
begin;
...
SELECT m.id AS message_id, m.title AS message_title, m.message AS message_text, m.date_add AS message_date, m.date_modify as date_modify, m.subdomain_id, s.url AS subdomain,
m.user_id  user_id,
...
l.id as location_id, l.latitude, l.longtitude, l.name AS location_name, l.region_id, r.name AS region_name, 
...
array(select id from multimedia where message_id=m.id) multimedia_id
FROM "Message" m
...
update config set value='0.51' where key='db_version';
commit;
