...
function getListItems($form)
{
    ...
    value = $('#filterString', $form).val();
    if(value)
        filterVars.searchString = value;

    value = $('#filterOrderBy', $form).val();
    if(value)
        filterVars.filterOrderBy = value;

    value = $('#orderByDir', $form).val();
    if(value)
        filterVars.orderByDir = value;

    return $.ajax('/ajax_forms/ajaxGetUsers', {data: filterVars,
                                               type: 'post',
                                               async: true,
                                               dataType: 'json',
                                               cache: false});
             ...
             $list.fadeOut(250, function(){
                 if(resp && resp.status == 'success') {
                     ...
                     if(resp.data.length > 0) {
                         ...
                         $list.empty();
                         $listPages.data('current-page', 1);
                         $itemsCount.html('0').fadeIn(250);
                         $formMessage.html(LANG_USERS_NOT_FOUND).fadeIn(250);
                     }
                     $list.fadeIn(250);
